...
export default function MobileNav({ links, c }) {
  ...
  const href = p => p === 'estimate' ? '/estimate' : p === 'contact' ? '/contact' : `/${p}`
  return <>
    ...
    <div className={`mobileMenu${open?' open':''}`} onClick={close}>
      <div className="mobileDrawer" onClick={e=>e.stopPropagation()}>
        ...
        <Link href="/contact" className="mobileCta" onClick={close} style={{marginTop:8}}>Contact Us</Link>
        <Link href={c.pages?.estimate?.enabled!==false?'/estimate':'/contact'} className="mobileCta" onClick={close} style={{background:'var(--btnBg,#2f7d32)',color:'var(--btnText,#fff)',marginTop:8}}>Get Quote</Link>
