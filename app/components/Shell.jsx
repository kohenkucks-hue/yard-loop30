  ...
  return <>
    ...
    <header className="nav">
      <Link className="brand" href="/">
        <div className={`logoMark ${c.brand.logoImage?'withLogo':''}`}>{c.brand.logoImage ? <img src={c.brand.logoImage} alt={c.brand.name} width={52} height={52}/> : c.brand.logoText}</div>
        <span><b>{c.brand.name}</b><small>{c.brand.tagline}</small></span>
      </Link>
      <nav className="navLinks" aria-label="Main navigation">
        {links.slice(0,6).map(l => <Link href={l.href} key={l.key} prefetch={true}>{l.label}</Link>)}
        <Link className="pill" href={isVisible(c,'estimate') ? '/estimate' : '/contact'} prefetch={true}>Get Quote</Link>
      </nav>
  ...
  return <footer>
    <div className="footerGrid">
      <div className="footerBrand">
        <div className="footerLogo"><div className={`footerLogoMark ${c.brand.logoImage?'withLogo':''}`}>{c.brand.logoImage ? <img src={c.brand.logoImage} alt={c.brand.name}/> : c.brand.logoText}</div><b>{c.brand.name}</b></div>
        <p>{c.brand.tagline}</p>
      ...
      <div className="footerCol"><h4>Pages</h4>{links.slice(0,8).map(l => <Link href={l.href} key={l.key}>{l.label}</Link>)}{isVisible(c,'legal') && <Link href="/legal">Legal</Link>}<Link href="/admin">Admin Login</Link></div>
      <div className="footerCol"><h4>Get Started</h4>{isVisible(c,'estimate') && <Link href="/estimate">Build Estimate</Link>}<Link href="/contact">Contact Yard Loop</Link><a href={phoneHref(c.brand.phone)}>Call Us</a>{isVisible(c,'contractor') && <Link href="/contractor">Contractor Partners</Link>}{c.googleReviewUrl && <a href={c.googleReviewUrl} target="_blank" rel="noreferrer">⭐ Leave a Google Review</a>}</div>
...
export function FinalCta({ c }){
  return <section className="offer">{c.brand?.logoImage && <img className="ctaLogo" src={c.brand.logoImage} alt={c.brand.name}/>}<p className="eyebrow light">Next step</p><h2>{c.finalCta?.headline}</h2><p>{c.finalCta?.text}</p><Link className="btn" href="/estimate">{c.finalCta?.button}</Link></section>
}
